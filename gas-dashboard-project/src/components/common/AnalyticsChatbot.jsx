...
function shortenText(text, maxChars = 420) {
  if (!text) return '';
...
function buildLocalFallbackResponse(question, context) {
  ...
  if (normalizedQuestion.includes('influence') || normalizedQuestion.includes('driver') || normalizedQuestion.includes('what influences')) {
    return [
      ...
      'Confirm with logs/inspection before action.',
    ].filter(Boolean).join('\n');
  }
  ...
  return [
    ...
    'Ask: “what influences X?” for targeted drivers.',
  ].filter(Boolean).join('\n');
}
      ...
      messages: [
        {
          role: 'system',
          content:
            'You are Analytics Copilot for a smart-kitchen dashboard. Answer only with route-relevant insights from provided context. Keep responses short (max 80 words), decision-oriented, and avoid unrelated details.',
        },
        {
          role: 'system',
...
export default function AnalyticsChatbot() {
  ...
  const sendMessage = async (promptText) => {
    ...
    try {
      ...
      setMessages((prev) => [...prev, { id: `a-${Date.now()}`, role: 'assistant', text: shortenText(responseText) }]);
    } catch (error) {
      const fallbackText = `${buildLocalFallbackResponse(question, context)}\n\n(Local fallback used: remote LLM unavailable.)`;
      setMessages((prev) => [...prev, { id: `a-${Date.now()}`, role: 'assistant', text: shortenText(fallbackText) }]);
    } finally {
      setIsSending(false);
    }
  };
