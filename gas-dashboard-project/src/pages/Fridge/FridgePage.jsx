...
export default function FridgePage() {
  ...
  const rawDuration = getFirstValid(
    latest?.Duration,
    latest?.duration,
    analysis?.Duration,
  ...
  const durationSeconds = getFirstValid(
    extractSeconds(rawDuration),
    analysis?.door_open_duration_sec,
    0
  );
  ...
  const durationLabel =
    typeof rawDuration === 'string'
      ? rawDuration
      : `${Math.floor(Number(durationSeconds) || 0)} sec`;

  const doorStatus = String(
    getFirstValid(latest?.status, latest?.doorStatus, 'UNKNOWN')
  ).toUpperCase();

  const openCountToday = Number(
    getFirstValid(analysis?.open_count_today, analysis?.open_count, 0)
  );

  const openCountYesterday = Number(
    getFirstValid(
      analysis?.open_count_yesterday,
      Math.max(openCountToday - 2, 0)
    )
  );

  const batteryLevel = Math.min(
    100,
    Math.max(0, Number(getFirstValid(latest?.battery, analysis?.battery_level, 98)))
  );

  const lastSignal = getFirstValid(latest?.last_signal, analysis?.last_signal, '--');
  const signalStrength = getFirstValid(
  ...
  const lastOpened = formatTimeDisplay(
    getFirstValid(latest?.last_opened, analysis?.last_opened, latest?.timestamp)
  );

  const timelineData = history.map((item) => ({
    ...item,
    value: String(item.status).toUpperCase() === 'OPEN' ? 1 : 0,
  }));

  const openEvents = history.filter(
  ...
  return (
    <div className="fridge-design">
      ...
      <div className="fridge-system-row">
        <div className="fridge-system-pill">
          System status : {doorStatus === 'OPEN' ? 'Warning' : 'Safe'}
        </div>
      ...
      <div className="fridge-top-grid">
        ...
        <div className="fridge-card">
          ...
          <div className="fridge-signal-text">
      ...
      <div className="fridge-mid-grid">
        ...
        <div className="fridge-card">
          ...
          <button type="button" className="fridge-alert-btn">
            Notify Staff
          </button>
        </div>
      </div>
            ...
            rows={[...history]
              ...
              .map((row) => ({
                ...
                Alert: getFirstValid(
                  row.Alert,
                  row.alert,
                  doorStatus === 'OPEN' ? 'High Open' : 'false'
                ),
                PowerImpact: getFirstValid(
                  row.PowerImpact,
                  row.power_impact,
